...
export default function SchedulePage() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      // Fetch data
      fetchLessons(userData.id, userData.role)
    ...
    try {
      ...
      if (upcomingResponse.ok) {
        ...
        const transformedUpcoming = upcomingData.map((lesson: any) => ({
          ...
          startTime: new Date(lesson.start_time).toLocaleTimeString('en-US', { 
            hour: '2-digit', 
            minute: '2-digit',
            hour12: true 
          }),
          endTime: new Date(lesson.end_time).toLocaleTimeString('en-US', { 
            hour: '2-digit', 
            minute: '2-digit',
            hour12: true 
          }),
          status: lesson.status === 'scheduled' ? 'confirmed' : lesson.status,
  ...
  const handleSaveAvailability = async () => {
    ...
    setIsSaving(true)
    try {
      // Transform availability to the format expected by the booking page
      // Convert from { monday: { available: true, slots: [{start, end}] } }
      // to [{ day: "Monday", slots: ["9:00 - 10:00"] }]
      const availabilityArray = Object.entries(availability)
        ...
        .map(([day, value]) => ({
          ...
          slots: value.slots.map(slot => `${slot.start} - ${slot.end}`)
        }))
